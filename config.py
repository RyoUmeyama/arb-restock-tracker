...
WATCH_ITEMS = [
    ...
    # ※ポケセンオンライン 抽選応募一覧(/lottery/apply.html)は監視から削除（2026-08-19）。
    #   Vueテンプレートで中身がJSレンダリングのため、抽選が公開されても静的HTMLは
    #   「公開中の抽選がありません」のまま変化せず、構造上どうやっても検知できない。
    #   それでいて取得は200で返り「抽出5行」あるため health[ok] に入り、
    #   日次ヘルスレポートでは“正常稼働中”として数えられていた（最悪の形の偽装）。
    #   実際 8/10〜8/14 のポケセン30th抽選をこの監視は1度も検知できなかった。
    #   代替は pokecen_news_ids（TOPページの記事ID差分＋本文取得）。
    ...
    {
        # ポケセンオンラインのニュース記事ID差分で告知を検知する（2026-08-19 追加）。
        # 抽選応募一覧(/lottery/apply.html)はVueテンプレートでJSレンダリングのため、
        # 抽選が公開されても静的HTMLは「公開中の抽選がありません」のまま変化しない＝検知不能。
        # 実害: 8/3公開の記事(id=20260803)に30th抽選の応募期間(8/10 12:00〜8/14 16:59)が
        # 載っていたが取りこぼし、メイン3商品(BOX/FUTURISTIC BOX/デッキセット)の応募機会を逸した。
        # TOPページには記事IDリンクが静的HTMLで並び、個別記事は静的に全文取得できる。
...
# 売却時に相場から差し引くコスト率。
# altemaの相場は【買取価格】＝店頭/宅配買取で受け取る手取り額であり、店側の利ざやは
# 既に差し引かれている。docs/15の出口優先順位も 1位=店頭買取(手数料・送料ゼロ)、
# 2〜3位=宅配買取(全手数料無料)で、メルカリ(10%)は7位・非推奨。
# よって既定は 0.0（二重控除の回避）。フリマ前提で保守的に見たい場合のみ 0.05〜0.10 にする。
# ※2026-07-29までは 0.10 固定で、買取価格からさらに10%を引く二重控除だった（実バグ）。
SELL_COST_RATE = 0.0
PROFIT_SPREAD_IN = 1.25    # 市場/定価 がこれ以上で監視ON（コスト控除後も利益が残る水準）
PROFIT_SPREAD_OUT = 1.05   # これ未満（定価割れ近傍）で監視から自動除外
POKECA_SPREAD_IN = 1.0     # ポケカは別格: 買取相場が定価以上なら監視ON（ほぼ全弾）
...
DIGEST_EXCLUDE_MARKERS = ["収録カードリスト", "当たりカード", "買取価格", "封入確率", "カードリスト",
                          ...
                          # nyuka-now集約ページの見出し・地の文・サイト宣伝（2026-08-21）。
                          # 「【在庫あり】…各種を再販実施中のストア」はページの状態バナーで、
                          # 【在庫あり】⇄【在庫なし】の切替のたびに新規行として誤通知されていた。
                          # 店舗名・商品・日時のない行は行動につながらない
                          "実施中のストア", "行っているストア", "確認できています",
                          "入荷Now", "プッシュ通知",
                          # 公認大会・イベントの参加抽選は商品入手と無関係（2026-08-31）。
                          # 「シティリーグ2027 事前抽選のエントリー期間延長」が
                          # 「抽選」語だけで通過して通知された
                          "シティリーグ", "チャンピオンズリーグ", "ジムバトル",
                          "トレーナーズリーグ", "オーガナイザー",
                          # 2026-08-31 一斉監査で追加。
...
# --- 発売カレンダー（A方針: 既存カテゴリの続弾を早く掴む）---
# ポケカ公式APIは「未来の発売日が確定した商品」を持っている（例: 2026-07-29時点で
# 10月16日発売分まで判明）。docs/17 の実データどおり新弾は繰り返し定価超の鞘を生むため、
# 発売前に把握できれば抽選応募・予約の機会を逃さない。
# 既存の「監視追加候補」が“相場が高い既発売品”を拾うのに対し、こちらは“これから出る物”を拾う。
RELEASE_CALENDAR_ENABLED = True
RELEASE_WINDOW_DAYS = 120      # 今日からこの日数以内に発売する商品を対象にする
RELEASE_MIN_PRICE = 3000       # これ未満はデッキシールド等の小物として除外（BOX/セットに限定）
RELEASE_MAX_LINES = 10         # 1回の通知に載せる最大件数
# 発売までの残日数の節目。ここをまたぐたびに再通知する（降順で保持すること）。
# 一度きりの通知だと、発売が近づいて実際に応募が始まる時期に何も知らせないため。
RELEASE_MILESTONES = [60, 30, 14, 7, 3, 1]
# 拡張パックは公式APIに【1パック価格】(200〜360円)で載るが、転売単位はBOX(1箱30パック前後)。
# 価格で足切りすると最重要カテゴリを取りこぼすため、これらは価格に関わらず必ず拾う。
RELEASE_ALWAYS_KEYWORDS = ["拡張パック", "ハイクラスパック", "強化拡張パック", "スペシャルセット"]
# 転売対象外の周辺グッズ（プレイマット・ファイル・スリーブ等）。
RELEASE_EXCLUDE_KEYWORDS = [
    "プレイマット", "ラバープレイマット", "デッキシールド", "デッキケース",
    "カードファイル", "コレクションファイル", "ディスプレイフレーム",
    "カードボックス", "ダメカン", "コイン", "サプライ",
]
...
# ヘルスレポートで「恒常障害（要確認）」とみなす連続取得失敗パス数。
# パスは約7分間隔なので8パス≒1時間連続失敗。それ未満は「断続・自動回復中」と表示
# （c-labo.jp等はクラウドIPに対して断続的にタイムアウトするが、大半のパスでは成功している）
PERSISTENT_FAIL_PASSES = 8

# フィルタ抑制ログの保持件数（毎朝のヘルスレポートで直近24h分を開示する）
SUPPRESS_LOG_KEEP = 200
